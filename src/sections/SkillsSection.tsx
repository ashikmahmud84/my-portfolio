...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-14 sm:py-20 lg:py-24 border-t border-white/[0.06] relative scroll-mt-20 sm:scroll-mt-24 overflow-hidden">
      ...
      />
      <div
        className="absolute bottom-1/4 left-1/5 w-[350px] max-w-full h-[350px] bg-cyan-500/[0.025] rounded-full blur-[120px] pointer-events-none"
        aria-hidden="true"
      />
      ...
      <Container size="wide" className="relative z-10">
        ...
        <div className="mb-10 sm:mb-16">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-5 pb-3 border-b border-white/[0.06]">
            <div className="flex items-center gap-2.5">
              <span className="p-1.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shrink-0">
                <Cpu className="w-4 h-4" />
              </span>
              <div>
                <h3 className="text-base sm:text-lg font-bold text-neutral-100 flex items-center gap-2 flex-wrap">
            ...
            {coreStackItems.map((item) => {
              ...
              return (
                <GlassCard
                  key={item.name}
                  variant="default"
                  className="relative p-4 sm:p-6 rounded-2xl border-white/[0.09] bg-[#0c0f17]/90 hover:border-emerald-500/40 hover:bg-[#101420] transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl flex flex-col justify-between"
                >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className="w-11 h-11 sm:w-13 sm:h-13 rounded-xl flex items-center justify-center bg-[#151926] border border-white/[0.1] shadow-inner group-hover:scale-105 transition-transform duration-300 shrink-0"
                        style={{
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="text-base sm:text-lg font-bold text-neutral-100 group-hover:text-white transition-colors truncate">
                          {item.name}
                        </h4>
                        <p className="text-xs font-mono text-emerald-400/90 font-medium truncate">
                          {item.role}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Description */}
                    <p className="text-xs text-neutral-400 leading-relaxed mb-4 text-pretty">
                      {item.description}
                    </p>
                  </div>
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-white/[0.06]">
            ...
            {/* Filter Buttons: Horizontal scrollable on small screens */}
            <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto max-w-full pb-1 pt-0.5 scrollbar-none -mx-1 px-1">
              <button
                type="button"
                onClick={() => setActiveTab('all')}
                className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-all cursor-pointer whitespace-nowrap border shrink-0 ${
                  activeTab === 'all'
              ...
              {skillCategories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono transition-all cursor-pointer whitespace-nowrap border shrink-0 ${
                      isActive
                        ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/40 shadow-sm font-semibold'
                        : 'bg-[#10141f] text-neutral-400 border-white/[0.08] hover:text-neutral-200 hover:border-white/20'
                    }`}
        ...
        <div className="space-y-8 sm:space-y-12">
          {getFilteredCategories().map((category) => (
            <motion.div
              ...
              className="rounded-2xl p-3.5 xs:p-5 sm:p-6 lg:p-7 bg-[#0b0e16]/80 border border-white/[0.07]"
            >
              {/* Category Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-4 sm:mb-5 pb-3 sm:pb-4 border-b border-white/[0.05]">
                <div className="flex items-center gap-3">
                  <span className="p-2 rounded-lg bg-white/[0.03] border border-white/[0.08] shrink-0">
                    {getCategoryIcon(category.iconName)}
                  </span>
                  <div>
                    <h4 className="text-base sm:text-lg font-bold text-neutral-100 flex items-center gap-2 flex-wrap">
              ...
              {/* Responsive Grid of Skill Cards:
                  Small mobile (320px-480px): 2 columns
                  Tablet (768px-1024px): 3-4 columns
                  Desktop (1280px+): 4-6 columns
              */}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3.5 lg:gap-4">
        ...
        <div className="mt-10 sm:mt-12 pt-6 sm:pt-8 border-t border-white/[0.06] flex items-center justify-center">
          <div className="inline-flex items-center justify-center gap-2 sm:gap-2.5 px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-full bg-[#10131d]/90 border border-white/[0.08] shadow-sm text-xs sm:text-sm font-mono text-neutral-300 text-center max-w-full flex-wrap">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shrink-0" />
            <span className="text-center">{coreStackSummary}</span>
          </div>
        </div>
      </Container>
    </section>
